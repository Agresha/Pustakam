...
function LatestBooks() {
  ...
  useEffect(() => {
    fetchData();
  }, [selectedLanguage]); // Re-fetch data when the selected language changes

  const latestBooks = [...books]
    .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)) // Sort by latest date
    .slice(0, 10); // Get top 10 latest books
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-2xl">{getTranslation("Latest Books", selectedLanguage)}</h2>
